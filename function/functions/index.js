...
exports.notifyInvitedToMovieList = functions.firestore.document("lists/{list_id}").onUpdate((change, context) => {

    return sendNotiAboutMovieLists(change, context);

});

async function sendNotiAboutMovieLists(change, context) {
    ...
    newData.members.forEach((member) => {
        if (!oldData.members.includes(member)) {
            addedUserIds.push(member);
            var tokens = await(db.collection("users").doc(member).collection("tokens").get()).map((doc) => {
                return doc.data().token;
            });
            addedTokens.concat(tokens);
        }
    });

    var notiTitle = "you have been added to new movie list";
    var notiBody = "you have been added to " + newData.title + " movie list";
    ...
    if (addedTokens.length > 0) {
        ...
        var res = await db.collection("noti").doc(t).set(data);
        ...
        return admin.messaging().sendToDevice(addedTokens, payload);
    } else {
        return;
    }
}
